...
对于任何**重大功能或重构工作**（即超出局部修复或单文件编辑范围的工作），请遵循以下工作流：

1. **讨论。** 在动手修改代码或规格之前，先就范围和方案达成一致。
2. **英文一次性路线图。** 在名为 `docs/throwaway-*.md` 的临时规划笔记中记录方案。该笔记记载里程碑模型、逻辑变更的有序清单以及预期的提交顺序。此类笔记仅用英文撰写（属于工作产物，豁免双语要求），并须包含 SPDX 许可证头以及一行 `**Status:**` 说明，注明其为临时文件，将在工作完成后删除或并入永久文档。
3. **实施。** 按照路线图，**每个逻辑变更一次提交**，逐项完成，随后删除该一次性笔记（或将其并入 `docs/next-steps.md` 或相关设计文档）。

这样可使大型变更易于评审，使方案在执行前可供检视，并防止未完成的规划文字泄漏到已发布的设计文档中。
...
本项目倾向于做小的原子提交：先做构建系统变更/重构，再做功能开发/修复。
如果一个重构可以拆成几个步骤，那就拆开：例如，先批量重命名一些类型，再给一个 trait 方法全局增加一个参数。
只要有这种可能：提交的原子性不受破坏，就请将大的 diffs 拆成几个小提交。
