...
**在文档文件中**（Markdown、README、设计文档），扫描器会抑制出现在围栏代码块内部、行内反引号内部或紧跟引号字符之后的标记。在行文中*提到*标记时，请使用反引号：`` `@liyi:module` ``。*使用*标记作为真实指令时，保持其裸写形式：`<!-- @liyi:requirement(name) -->`。详见设计文档中的*自托管与自引用问题*一节。
