...
This is not only an AI-era problem — it is a durability problem. Even well-managed teams with disciplined project management (JIRA epics linked to Confluence design docs, every PR tagged to a ticket) face knowledge decay: platform migrations lose history (Redmine → JIRA without redirect links or change logs), document owners leave and their pages become read-only, design docs are superseded without the older versions being updated, and organizational restructuring reassigns permissions and scatters institutional memory. The 5% of decisions that still matter after years — billing semantics, settlement rules, transaction type inventories — are precisely the ones that live in the most fragile layer. The code outlives every knowledge system that was supposed to explain it. For the intern who joins in year seven and inherits a payment service written by someone who left in year three, with Confluence pages they can't edit and JIRA tickets referencing a Redmine ID that no longer resolves, the only reliable source of truth is the repository itself.
